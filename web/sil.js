/*
 * Sil-Q in the browser: terminal rendering, input and save persistence.
 * The game (sil-core.js / .wasm) calls into Module.qb (see src/main-web.c).
 * Copied from the RVIP web template (Quickband).
 */
(function () {
	'use strict';

	var TILE = 16;                 /* source tile size in tiles.png (MicroChasm 16x16) */
	var PERSIST = ['/sil-q/lib/save', '/sil-q/lib/user', '/sil-q/lib/apex'];
	...
	var TERMS = [
		...
		{ id: 'mon', title: 'Monster list' },
		{ id: 'rec', title: 'Recall' },
		{ id: 'itm', title: 'Combat rolls' },
	 ...
	 *   |       main       +-------+   mon    : y of mon | itm (of top height)
	 *   |                  |  mon  |   bottom : y of top | bottom row
	 *   |                  +-------+   msg    : x of msg | rec
	 *   |                  |  itm  |
	 *   +-------------+----+-------+
	 *   |     msg     |    rec     |
	...
	var LAYOUT_FILE = '/sil-q/lib/user/web-layout.json';
	...
	function makeWM() {
		...
		wm = RvipWM({
			...
			wins: [{ id: 'main', title: 'Map' }, { id: 'inv', title: 'Inventory' }, { id: 'msg', title: 'Messages' }, { id: 'mon', title: 'Monster list' }, { id: 'rec', title: 'Recall' }, { id: 'itm', title: 'Combat rolls' }, { id: 'eqp', title: 'Equipment' }],
	...
	/* Sound effects (lib/xtra/sound/sound.cfg) and music, both off by default */
	...
	function loadSoundCfg() {
		audio.cfg = {};
		var t = '';
		try { t = Module.FS.readFile('/sil-q/lib/xtra/sound/sound.cfg', { encoding: 'utf8' }); } catch (e) { }
		t.split('\n').forEach(function (l) {
	...
	function updateMusic() {
		/* Sil-Q has no town and no music of its own: loop the tune while a game is on */
	...
	var qb = {
		...
		nextEvent: function (atCmd) {
			...
			return events.shift().k;
		},

		plog: function (msg) {
			console.warn('[sil-q]', msg);
			status(msg, true);
		},
	...
	function mountPersistent() {
		...
		FS.syncfs(true, function (err) {
			...
			/* Continue the last saved character (uid 0 in Emscripten) */
			var p = saveFilePath();
			/* push: Emscripten keeps a reference to this array for main() */
			if (p) Module.arguments.push('-u' + p.split('/').pop().replace(/^\d+\./, ''));
	...
	function removeSaves() {
		listFiles().forEach(function (p) {
			if (p.indexOf('/sil-q/lib/save/') === 0) Module.FS.unlink(p);
		});
	}

	/* The most recently saved character (files are <uid>.<name>) */
	function saveFilePath() {
		var FS = Module.FS;
		var files = listFiles().filter(function (p) {
			return p.indexOf('/sil-q/lib/save/') === 0 && /^\d+\.[^.]+$/.test(p.split('/').pop());
		});
	...
	window.Module = {
		qb: qb,
		/* Set once IndexedDB is loaded: -u<name> of the last saved character */
		arguments: [],
		noInitialRun: false,
		preRun: [function () {
			if (!tilesDone) {
			...
			Module.FS.mkdirTree('/sil-q/lib/data'); Module.FS.chdir('/sil-q');
			mountPersistent();
		}],
