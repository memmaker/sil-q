...
/*
 * Browser (Emscripten/WASM) front end for Sil-Q (RVIP web port, copied
 * from Quickband's main-web.c).
 *
 * All drawing is done by JavaScript on one <canvas> per term (see
 * web/sil.js).  Blocking input uses Asyncify: when the game waits
...
/* ---- JavaScript side (implemented in web/sil.js) ---- */
 ...
 * command prompt, where a queued ^R (redraw) repaints everything (menus and
 * prompts elsewhere don't expect a size change).  Returns TRUE if a key
 * was queued.
 */
static bool web_apply_layout(void)
{
	...
	for (i = 0; i < WEB_TERMS; i++)
	{
		...
		/* The main window needs the map redrawn for its new size */
		if (!i && at_prompt) queued = TRUE;
	}
	...
	if (subs && character_generated)
	{
		...
		p_ptr->window |= (PW_INVEN | PW_EQUIP | PW_PLAYER_0 | PW_COMBAT_ROLLS |
		                  PW_MONSTER | PW_OBJECT | PW_MESSAGE | PW_MONLIST);
		if (at_prompt) queued = TRUE;
	}
...
/* Sound events go to the page, which plays web/sound/sound.cfg */
...
static errr Term_xtra_web(int n, int v)
{
	switch (n)
	{
		...
		case TERM_XTRA_SOUND: web_sound(v); return (0);
		case TERM_XTRA_FRESH:
			...
			/* The page plays music while a game is on */
...
errr init_web(int argc, char **argv)
{
	...
	/* MicroChasm tiles in big-tile mode, as in the X11 build (-g -b) */
